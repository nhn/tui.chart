...
import { deepCopy, getFirstValidValue, includes, isNumber, isUndefined, range, } from "../helpers/utils";
    ...
    action: {
        setSeriesData({ state, initStoreState }) {
            ...
            Object.keys(rawSeries).forEach((seriesName) => {
                ...
                let originSeriesData = rawSeries[seriesName].map((m, idx) => (Object.assign(Object.assign({}, m), { rawData: m.data, data: getSeriesDataInRange(m.data, rawCategories, seriesName, zoomRange), color: colors ? colors[idx] : '' })));

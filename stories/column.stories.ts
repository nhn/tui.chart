...
export const dataLabels = () => {
  ...
  const rotation = number('rotation', 0, { range: true, min: 0, max: 360, step: 15 });
  const scriptableColor = boolean('scriptable color', false);
  const textColor = color('label color', '#333333');
  const colorFunc = function (data) {
    return data.value >= 9000 ? '#fff' : textColor;
  };
  const labelColor = scriptableColor ? colorFunc : textColor;

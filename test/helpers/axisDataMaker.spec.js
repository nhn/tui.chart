...
describe('axisDataMaker', function() {
    ...
    describe('_getCandidateTickCounts()', function() {
        ...
        it('tickCount인자가 넘어오면 tickCount를 배열로감싸 반환합니다', function() {
            var tickCounts = maker._getCandidateTickCounts({}, null, 5);
            expect(tickCounts).toEqual([5]);
        });
    });
    ...
    describe('makeValueAxisData()', function() {
        ...
        it('stacked 옵션이 "normal"인 value 타입 axis data를 생성합니다.' +
            'limit.max는 행(ex: [60, 30]) 의 합산값에 영향을 받습니다.', function() {
            var result = maker.makeValueAxisData({
                values: [
                    [70, 10],
                    [20, 20],
        ...
        it('stacked 옵션이 "percent"인 값 타입의 axis data를 생성합니다.' +
            'limit은 %에 대한 정보를 제공하며 항상 [0, 25, 50, 70, 100]의 레이블을 제공합니다.', function() {
            var result = maker.makeValueAxisData({
                values: [
                    [70, 10],
                    [20, 20],
